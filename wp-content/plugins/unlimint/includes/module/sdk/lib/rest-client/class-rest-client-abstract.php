...
class Unlimint_Rest_Client_Abstract {
	 ...
	 * @param string $urlPrefix URL.
	 *
	 * @return array|null
	 */
	public static function exec_abs( $request, $urlPrefix ) {
		$logger = new WC_Unlimint_Logger();
		$logger->info( __FUNCTION__, 'URL: ' . wp_json_encode( $urlPrefix, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE ) );

		try {
			$connect = self::build_request( $request, $urlPrefix );
			if ( ! $connect ) {
				return null;
			}

			$response = self::execute( $connect );
			$logger->info( __FUNCTION__, 'Response: ' . wp_json_encode( $response, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE ) );
	 ...
	 * @throws WC_Unlimint_Exception Build request exception.
	 */
	public static function build_request( $request, $urlPrefix ) {
		 ...
		 */
		$connect = curl_init();
		curl_setopt( $connect, CURLOPT_USERAGENT, 'UnlimintPlugin/' . WC_Unlimint_Constants::VERSION . '/Woocommerce' );
		...
		curl_setopt( $connect, CURLOPT_URL, $urlPrefix . $request['uri'] );
		...
		if ( $json_content ) {
			...
			if ( function_exists( 'json_last_error' ) ) {
				...
				if ( JSON_ERROR_NONE !== $json_error ) {
					throw new WC_Unlimint_Exception( "JSON Error [$json_error] - Data: " . $request['data'] );
				}
			}
		} elseif ( $form_content ) {
	 ...
	 *
	 * @return array|null
	 * @throws WC_Unlimint_Exception Execute call exception.
	 */
	public static function execute( $connect ) {
		$response   = null;
		$api_result = curl_exec( $connect );
		if ( curl_errno( $connect ) ) {
			throw new WC_Unlimint_Exception( curl_error( $connect ) );
		}
	...
	private static function validate_request( $request = [] ) {
		if ( ! extension_loaded( 'curl' ) ) {
			throw new WC_Unlimint_Exception( 'cURL extension not found. You need to enable cURL in your php.ini or another configuration you have.' );
		}

		if ( ! isset( $request['method'] ) ) {
			throw new WC_Unlimint_Exception( 'No HTTP METHOD specified' );
		}

		if ( ! isset( $request['uri'] ) ) {
			throw new WC_Unlimint_Exception( 'No URI specified' );
		}
	}
}
