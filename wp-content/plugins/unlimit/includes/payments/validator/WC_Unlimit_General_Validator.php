...
class WC_Unlimit_General_Validator {
	 ...
	 * @param $addressType
	 */
	private function validate_zip( $paymentMethod, $errorMessage, $maxLength, $addressType, $addressField, $fieldValue ) {
		if ( $paymentMethod === 'woo-unlimit-ticket' && $errorMessage === self::STR ) {
		...
		if ( ( $paymentMethod === 'woo-unlimit-custom' || $paymentMethod === 'woo-unlimit-pix' ) &&
		     $errorMessage === self::STR && strlen( $fieldValue ) > $maxLength ) {
			wc_add_notice(
				__( self::STRONG_TAG . ucfirst( $fieldValue ) .
				    ": $errorMessage</strong> must be $maxLength characters.", 'unlimit' ),
				'error'
			);
		}
	}
