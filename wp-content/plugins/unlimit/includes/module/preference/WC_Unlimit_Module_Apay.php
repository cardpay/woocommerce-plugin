...
class WC_Unlimit_Module_Apay extends WC_Unlimit_Alt_Module {
	...
	public function __construct( $payment, $order, $post_fields ) {
		parent::__construct( $payment, $order, $post_fields, 'APPLEPAY', 'apay' );

		if ( ! empty( $_POST["cardpay_custom_apay"]["signature"] ) ) {
			$this->api_request[ WC_Unlimit_Constants::PAYMENT_DATA ]['encrypted_data'] =
				base64_encode(
					stripcslashes( $_POST["cardpay_custom_apay"]["signature"] )
				);
		}
	}
}
