...
class WC_Unlimit_Module_Custom extends WC_Unlimit_Module_Abstract {
	...
	private function build_api_request() {
		...
		$are_installments_enabled = (
			'yes' === get_option( $field_name_prefix . WC_Unlimit_Admin_BankCard_Fields::FIELD_INSTALLMENT_ENABLED )
		);
		$installments             = (int) $card_post_fields[ self::INSTALLMENTS ];
		$installment_type         = get_option(
			$field_name_prefix .
		...
		$is_api_access_mode = ( 'gateway' === get_option(
				$field_name_prefix . WC_Unlimit_Admin_BankCard_Fields::FIELD_API_ACCESS_MODE
			) );
